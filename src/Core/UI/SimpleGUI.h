...
public:
    ...
    MOE::Graphics* GetGraphics() const
	{
		return g;
	}
...
protected:
	...
	void beginUI()
	{
		...
		m_defshader->Unbind();
		g->BindTexture(VG_TEXTURE_2D, m_deftex->GetID());
	}
	void endUI()
	{
		m_defvb->Update();
		...
		g->BlendFunc(VG_ONE, VG_ONE);//_MINUS_SRC_ALPHA);
        m_defshader->Bind();
		m_defshader->SetUniform("mono",1.0f);
		m_defshader->Unbind();
		const int n = static_cast<int>(m_drawsets.size());
		for (int i = 0; i < n; ++i) {
			g->BindTexture(VG_TEXTURE_2D, m_drawsets[i].tex->GetID());
