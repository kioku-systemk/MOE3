...
{
public:
	SimpleVB(MOE::Graphics* sg, SimpleShader* shader)
	{
		g = sg;
		m_color = 0xFFFFFFFF;
		m_uv[0] = 0.0f;
	...
	void Update()
	{
		...
		g->BufferData(VG_ELEMENT_ARRAY_BUFFER, static_cast<int>(m_indexbuffer.size()) * sizeof(unsigned short), &m_indexbuffer[0], VG_STATIC_DRAW);
	...
	void Draw()
	{
		...
		g->VertexAttribPointer(m_att_pos, 3, VG_FLOAT, VG_FALSE, sizeof(VertexFormat), 0);
		g->EnableVertexAttribArray(m_att_col);
		g->VertexAttribPointer(m_att_col, 4, VG_UNSIGNED_BYTE, VG_TRUE, sizeof(VertexFormat), (const void*)(sizeof(float)*3));
		g->EnableVertexAttribArray(m_att_uv);
		g->VertexAttribPointer(m_att_uv, 2, VG_FLOAT, VG_FALSE, sizeof(VertexFormat), (const void*)(sizeof(float)*4));
		//g->DrawArrays(VG_TRIANGLES, 0, static_cast<int>(m_buffer.size()));
        g->DrawElements(VG_TRIANGLES, static_cast<int>(m_indexbuffer.size()), VG_UNSIGNED_SHORT, 0);
