...
{
public:
	SimpleShader(MOE::Graphics* sg, const char* vsshader, const char* fsshader)
	{
		g = sg;
		m_program = 0;
		m_oldProgram = 0;
		m_binding = false;
		const int vshader = createShader(vsshader, VG_VERTEX_SHADER);
		const int fshader = createShader(fsshader, VG_FRAGMENT_SHADER);
	...
	void Bind()
	{
		g->GetIntegerv(VG_CURRENT_PROGRAM, reinterpret_cast<int*>(&m_oldProgram));
...
private:
	void printShaderInfoLog(MOE::Graphics* g, int shader) const
	{
		int bufSize = 0;
	...
	void printProgramInfoLog(MOE::Graphics* g, GLuint program) const
	{
		int bufSize;
	...
	int createShader(const char* programSource, MOE::VGenum shaderType) const
	{
		...
		g->GetShaderiv(shader, VG_COMPILE_STATUS, &compiled);
